...
## 17. UI-to-API mapping

The v3 frontend is organized as functions in `python/gui_web/web/app.js`. The following mapping records the actual API calls found in the current file.

| UI surface/function | API calls |
| --- | --- |
| First-run password | `complete_first_run_setup` |
| Wizard router/device/schedule/branding | `get_setup_wizard_step`, `get_session_state`, `get_connection_status`, `complete_setup_device_step`, `complete_setup_schedule_step`, `complete_setup_branding_step` |
| Role authentication/session | `authenticate_role`, `set_current_role`, `lock_session`, `change_admin_password` |
| Connection controls | `get_settings`, `connect`, `disconnect`, `get_connection_status` |
| Serial troubleshooting | `get_serial_troubleshooting`, `open_device_manager`, `open_driver_help` |
| Scan control | `start_scan`, `stop_scan` |
| Dashboard | `get_connection_status`, `get_dashboard_stats`, `get_recent_activity`, `request_fingerprint_count` |
| Attendance page | `get_attendance`, `get_attendance_evaluation`, `export_attendance_csv`, `export_attendance_evaluation_csv` |
| Students page | `get_students`, `get_student`, `delete_on_device`, `delete_student`, `save_student`, `validate_student_fields`, `cancel_enroll`, `discard_enrollment`, `start_enroll`, `export_students_csv` |
| Student enrollment save | `delete_on_device`, `save_student`, `request_fingerprint_count` |
| Wipe controls | `wipe_all_on_device`, `wipe_all_data` |
| Reports page | `get_dashboard_stats`, `get_statistics_report`, `export_statistics_report` |
| Backup controls | `list_backups`, `create_backup`, `restore_backup` |
| Logs page | `get_app_log` |
| Device reset/raw diagnostics | `reset_device`, `send_serial_command` |
| Calendar page | `get_calendar_month`, `set_calendar_entry`, `remove_calendar_entry` |
| Settings page | `get_settings`, `get_session_state`, `get_connection_status`, `list_ports_detailed`, `forget_saved_port`, `save_ui_settings`, `restore_default_settings`, `open_log_folder` |
| Activity/session refresh | `touch_session`, `get_settings`, `get_session_state`, `get_connection_status` |

### Main event-consumer surface

`window.dsisEvent` in `app.js` dispatches backend push events to scan, serial, connection, enrollment/delete/wipe, fingerprint count, data refresh, mode, and log UI handlers.
...
## 18. Inventory maintenance rule

When a new public Api method, UI page/modal, data file, firmware command, core/service module, or behavior-specific test is added or removed, update this inventory in the same change.
