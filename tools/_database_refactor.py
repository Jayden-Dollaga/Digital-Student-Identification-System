...
class AttendanceRow(RowDict, total=False):
    id: int
    fingerprint_id: int
    student_no: str
    student_name: str
...
class StudentRow(RowDict, total=False):
    fingerprint_id: int
    student_no: str
    student_name: str
    grade: str
...
def get_attendance_today() -> List[AttendanceRow]:
    ...
    query = f"{ATTENDANCE_JOIN_QUERY} WHERE a.date = ? ORDER BY a.timestamp DESC"
    with get_connection() as conn:
...
def get_attendance_paginated(limit: int = 100, offset: int = 0) -> List[AttendanceRow]:
    query = f"{ATTENDANCE_JOIN_QUERY} ORDER BY a.timestamp DESC LIMIT ? OFFSET ?"
    with get_connection() as conn:
